...
Section TailRecrsion.
    ...
    (Hbody : forall v, hlist.foralls (fun g => forall t m mc, tuple.foralls (fun l =>
      ...
      (word.unsigned br <> 0%Z -> cmd call c t m localsmap mc'
        (fun t' m' localsmap' mc'' =>
          ...
          match tuple.apply (hlist.apply (spec v') g' t' m') l'
                      (addMetricInstructions 2
                      (addMetricLoads 2
                      (addMetricJumps 1 mc''))) with S' =>
          S'.(1) /\ Markers.right (
            lt v' v /\
            forall T M, hlist.foralls (fun L => forall MC, tuple.apply (S'.(2) T M) L MC -> tuple.apply (S_.(2) T M) L MC)) end))))))))) /\
      (word.unsigned br = 0%Z -> tuple.apply (S_.(2) t m) l
            (addMetricInstructions 1
            (addMetricLoads 1
            (addMetricJumps 1 mc')))))))end))))
  ...
  Proof.
    ...
    { eapply Proper_cmd; [reflexivity..| | |eapply Hpc].
      ...
      pose proof fun T M => hlist.foralls_forall (HR T M) as HRR; clear HR.
      cbn in HRR. eauto 10. }
